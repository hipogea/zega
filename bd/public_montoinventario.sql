-- phpMyAdmin SQL Dump
-- version 4.3.8
-- http://www.phpmyadmin.net
--
-- Servidor: localhost
-- Tiempo de generación: 08-12-2015 a las 20:31:01
-- Versión del servidor: 5.5.42-37.1
-- Versión de PHP: 5.4.23

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8 */;

--
-- Base de datos: `neologys_razzo`
--

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `public_montoinventario`
--

CREATE TABLE IF NOT EXISTS `public_montoinventario` (
  `id` mediumint(9) NOT NULL,
  `dia` char(2) COLLATE utf8_unicode_ci NOT NULL,
  `mes` char(2) COLLATE utf8_unicode_ci NOT NULL,
  `anno` char(2) COLLATE utf8_unicode_ci NOT NULL,
  `iduser` int(11) NOT NULL,
  `montolibre` decimal(15,3) NOT NULL,
  `montoreserva` decimal(15,3) NOT NULL,
  `montotran` decimal(15,3) NOT NULL,
  `montodif` decimal(15,3) NOT NULL,
  `codal` char(3) COLLATE utf8_unicode_ci NOT NULL,
  `codcen` char(4) COLLATE utf8_unicode_ci NOT NULL,
  `codgrupo` varchar(4) COLLATE utf8_unicode_ci NOT NULL,
  `numitems` mediumint(9) NOT NULL,
  `fecha` date NOT NULL,
  `montototal` decimal(16,0) NOT NULL
) ENGINE=MyISAM AUTO_INCREMENT=162 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;

--
-- Índices para tablas volcadas
--

--
-- Indices de la tabla `public_montoinventario`
--
ALTER TABLE `public_montoinventario`
  ADD PRIMARY KEY (`id`);

--
-- AUTO_INCREMENT de las tablas volcadas
--

--
-- AUTO_INCREMENT de la tabla `public_montoinventario`
--
ALTER TABLE `public_montoinventario`
  MODIFY `id` mediumint(9) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=162;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
