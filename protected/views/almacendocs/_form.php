...
<div class="panelizquierdo">
		...
		<?php  
			...
			$datos1 = CHtml::listData(Almacenmovimientos::model()->findAll(array('order'=>'movimiento')),'codmov','movimiento');
		  	echo $form->DropDownList($model,'codmovimiento',$datos1, array('empty'=>'--Seleccione un movimiento--',  
					...
					echo CHtml::textField('sjfs',$model->almacendocs_almacenmovimientos->movimiento, array('disabled'=>'disabled','size'=>35));
				}									   
		?>
	...
	<div class="row">
		<?php echo $form->labelEx($model,'codtipovale'); ?>
		<?php   $datos= array('01'=>'Ingreso','02'=>'Salida')   ;       ?>
		<?php 
		// if(!$model->isNewRecord) { 
							echo $form->dropDownList($model,'codtipovale',$datos,array('empty'=>'--Seleccione Tipo'));
						//	} else {
								//echo CHtml::textField('sjfsss',($model->codtipovale=='01')?'Ingreso':'Salida',array('disabled'=>'disabled','size'=>35));

							//}

		 ?>
		<?php echo $form->error($model,'codtipovale'); ?>
	</div>
...
<div class="panelderecho">
		...
		<?php 
			if(!$model->isNewRecord)
		     ...
		     echo CHtml::textField('hola',
			  Estado::model()->find('codestado=:miestado and codocu=:midocumento',array(':midocumento'=>'101',':miestado'=>$model->cestadovale))->estado,
	...
	<?php  ////INSERTAMOS UN IDENTIFICADOR OCULTO PARA EVITAR EL DOBLE POST
	$this->widget('ext.identificador.identificador', array('nombrecampo'=>'identificadorpost','form'=>$form,'model'=>$model,));
	?>
	...
	<?php if ( !$model->isNewRecord )  {
				...
				$this->renderpartial('vw_detalle_vale',
